/* extension.js - GNOME 45+ / 50 compatible Crypto Ticker */
...
class CryptoTicker extends PanelMenu.Button {
    _init(extension) {
        ...
        this.container = new St.BoxLayout({ style_class: 'moving-container' });
    ...
    _refresh() {
        ...
        this._fetch(url, (err, body) => {
            if (err) {
                console.error(`crypto-ticker fetch error: ${err}`);
                this._setUnknown('API error');
                return;
            }
            try {
